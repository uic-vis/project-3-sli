...
function scatterplot9(data9) {
    ...
    svg.append("text")
        ...
        .text("Interactive Plot Between Fare and Miles of Trips in September");

    const g = svg.append('g')
        .attr('transform', `translate(${margin.left}, ${margin.top})`);

    const x = d3.scaleLinear()
        .domain(d3.extent(data9, d => d['Trip Miles'])).nice()
        .range([0, width])

    const y = d3.scaleLinear()
        .domain(d3.extent(data9, d => +d['Fare'])).nice()
        .range([height, 0])
    ...
    `
    const text = document.querySelector("#data9");
    text.innerHTML = content
    // update the data 
    svg.property('value', data9.filter(isBrushed)).dispatch('input');
    }
...
d3.csv("https://raw.githubusercontent.com/chloelili22/chloelili22.github.io/main/taxiTripsinSept.csv")
    .then(scatterplot9)
...
function multiplePlot9(data9) {
    function brushableScatterplot9() {
        var margin = { top: 30, right: 150, bottom: 80, left: 80 },
            width = 960 - margin.left - margin.right,
            height = 500 - margin.top - margin.bottom,
            tooltip = { width: 100, height: 100, x: 10, y: -30 };

        var taxiCompany = Array.from(new Set(data9.map(d=> d.Company)));
        ...
        //initialize margin end
        var svg = d3.select("#scatterplot9").append("svg")
            .attr("width", width + margin.left + margin.right)
            .attr("height", height + margin.top + margin.bottom)
            .append("g")
            .property('value', initialValue)
        ...
        svg.append("text")
            ...
            .attr("y", 13)
            .style("text-anchor", "middle")
            .text("Interactive Plot Between Fare and Tips of Trips in September");
        ...
        const y = d3.scaleLinear()
            .domain(d3.extent(data9, d => d['Tips'])).nice()
            .range([height, 0])
    ...
    function barChart9(){
        ...
        svg.append("text")
            ...
            .attr("y", 15)
            .style("text-anchor", "middle")
            .text("Bar Chart of Each Taxi Company in September");
    ...
    const scatter = brushableScatterplot9(data9);
    const bar = barChart9(data9);
